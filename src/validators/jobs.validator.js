...
const browseJobsSchema = Joi.object({
  ...
  // "For You" feed (JS-06): comma-separated list of categories,
  // matching a job posted under ANY of them — same convention as
  // seeker.validator.js's `skills` param. Takes priority over the
  // single `category` above when both are present (see
  // jobs.service.js#browseJobs); `category` is kept for existing
  // callers doing a single-category filter rather than the seeker's
  // full preference set.
  categories: Joi.string()
    ...
    .messages({
      'any.invalid': `categories must be a comma-separated list of: ${JOB_CATEGORY_KEYS.join(', ')}`,
    }),
