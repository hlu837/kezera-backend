...
const updateProfileSchema = Joi.object({
  ...
  // Physical office address — free text (see Agency.model.js#address),
  // shown alongside backoffice_phone in the public "Find agencies near
  // you" quick-view sheet.
  address: Joi.string().trim().max(500).allow('').optional(),
  // "Years in business" shown on the public directory/profile card
  // (see Agency.model.js#foundedYear) — the year the agency itself was
  // founded, not this account's registration date on the platform.
  // `.allow(null)` lets an agency clear a previously-set value the
  // same way `operational_city`/`bio` clear via `.allow('')`; Joi
  // doesn't accept `''` for a number field, hence `null` instead.
  ...
  .min(1)
  .messages({
    'object.min': 'Provide at least one of: agency_name, operational_city, backoffice_phone, address, bio, founded_year',
  })
 ...
 * Mirrors seeker.validator.js#updateLocationSchema exactly — both
 * coordinates required together, no partial form.
 */
 ...
 * Mirrors seeker.validator.js#nearbySeekersSchema's shape (minus the
 * skills/category/trade filters, which have no agency equivalent).
 */
const nearbyAgenciesSchema = Joi.object({
  latitude: Joi.number().min(-90).max(90).required(),
  longitude: Joi.number().min(-180).max(180).required(),
