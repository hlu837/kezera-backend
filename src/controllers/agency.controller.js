...
async function updateLocationHandler(req, res, next) {
  try {
    const profile = await agencyService.updateLocation(req.user.id, req.body);
    return res.status(200).json({ status: 'success', data: { profile } });
  } catch (err) {
    return next(err);
  }
...
/**
 * GET /api/v1/agencies/nearby — no auth, mirrors seeker.controller.js's
 * nearbyHandler. Query validated upstream by validateQuery(nearbyAgenciesSchema).
 */
async function nearbyHandler(req, res, next) {
  try {
    const result = await agencyService.nearbyAgencies({
