...
/** "Request Service": a ServiceRequest moved through created/assigned/accepted/declined/completed/cancelled. */
const NOTIFY_SERVICE_REQUEST_EVENT = 'NOTIFY_SERVICE_REQUEST_EVENT';
...
/**
 * Enqueues a NOTIFY_SERVICE_REQUEST_EVENT task. The worker picks it up
 * and calls `notifications.service.notifyServiceRequestEvent(serviceRequestId,
 * event, recipientUserId)` to SMS/email/in-app-notify whoever needs to
 * act or be informed next. No deterministic `jobId` here, same
 * reasoning as `enqueueNewMessageNotification` — every stage
 * (created/assigned/accepted/declined/completed/cancelled) is its own
 * one-off event, never a recomputation to dedupe.
 *
 ...
 * @returns {Promise<import('bullmq').Job>}
 */
async function enqueueServiceRequestNotification(serviceRequestId, event, recipientUserId) {
  return notificationsQueue.add(
    NOTIFY_SERVICE_REQUEST_EVENT,
    { service_request_id: serviceRequestId, event, recipient_user_id: recipientUserId },
    {
      attempts: 5,
      backoff: { type: 'exponential', delay: 10000 },
      removeOnComplete: { age: 3600, count: 1000 },
