...
const {
  ...
  NOTIFY_ACCOUNT_STATUS_CHANGED, NOTIFY_SERVICE_REQUEST_EVENT,
...
/**
 * Handles a NOTIFY_SERVICE_REQUEST_EVENT task: SMS's + emails +
 * in-app-notifies whoever needs to see this stage of a ServiceRequest.
 * @param {import('bullmq').Job} bullJob
 */
async function handleNotifyServiceRequestEvent(bullJob) {
  const {
    service_request_id: serviceRequestId, event, recipient_user_id: recipientUserId,
  } = bullJob.data || {};
  if (!serviceRequestId || !event || !recipientUserId) {
    throw new Error(
      'NOTIFY_SERVICE_REQUEST_EVENT task payload is missing required '
      + '"service_request_id"/"event"/"recipient_user_id"',
    );
  }

  const result = await notificationsService.notifyServiceRequestEvent(
    serviceRequestId,
    event,
    recipientUserId,
  );
  ...
  // eslint-disable-next-line no-console
  console.log(
    `[notifications-worker] service_request_id=${serviceRequestId} event=${event} `
    + `sms_sent=${result.smsSent} email_sent=${result.emailSent}`,
  );
...
async function processNotificationJob(bullJob) {
  switch (bullJob.name) {
    ...
    case NOTIFY_SERVICE_REQUEST_EVENT:
      return handleNotifyServiceRequestEvent(bullJob);
    default:
      throw new Error(
