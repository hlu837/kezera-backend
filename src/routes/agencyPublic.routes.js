...
const { nearbyHandler } = require('../controllers/agency.controller');
const { nearbyAgenciesSchema } = require('../validators/agency.validator');
...
// Public: the agency directory — "Agencies" tab on the landing page.
// One entry per approved/active agency with key stats (open jobs,
// rating). MUST stay above the '/:agencyId/*' routes below so it isn't
// shadowed, and (per this file's header note) above agency.routes.js's
// auth gate so a logged-out visitor can browse it.
router.get('/', validateQuery(listAgenciesQuerySchema), listAgenciesHandler);

// Public: "Find agencies near you" map/list, mirroring
// `GET /seekers/nearby`. MUST stay above `/:agencyId/*` below (though
// it wouldn't currently collide — no route here is a bare
// `/:agencyId` — keeping every literal path above the param routes is
// this file's existing convention, so `/nearby` follows it too).
router.get('/nearby', validateQuery(nearbyAgenciesSchema), nearbyHandler);
