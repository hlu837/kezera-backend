...
async function sendMessage(userId, role, placementId, body) {
  ...
  // Genuinely fire-and-forget: NOT awaited. The Redis connection behind
  // this queue is configured with `maxRetriesPerRequest: null` (a hard
  // BullMQ requirement — see config/queue.js), which means that if
  // Redis is down or slow, `.add()` never resolves *or* rejects, it
  // just retries in the background indefinitely. Awaiting it here used
  // to mean the whole HTTP request — and the sender's UI — would hang
  // forever waiting for a notification queue that has nothing to do
  // with whether the message itself was saved successfully.
  enqueueNewMessageNotification(message.id).catch((err) => {
