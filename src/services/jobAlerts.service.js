...
function buildAlertEmail({ jobTitle, jobLocation, savedSearchName }) {
  ...
  const htmlBody = `
    <p>A new job matching ${label} was just posted on Kezera.</p>
    <p><strong>${jobTitle}</strong> — ${jobLocation}</p>
    <p>Log in to your Kezera account to view and apply.</p>
  `.trim();
...
function buildCategorySmsMessage({ jobTitle, categoryLabel, jobLocation }) {
  return `New ${categoryLabel} job on Kezera: "${jobTitle}" in ${jobLocation}. Open the app to apply.`;
}
  ...
  const results = await Promise.allSettled(
    seekers.map(async (seeker) => {
      ...
      await writeInAppNotification(
        ...
        'job_alert',
        'New job alert',
        `New ${categoryLabel} job on Kezera: "${job.title}" in ${job.location}.`,
        { jobId },
      );
