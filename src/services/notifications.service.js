...
async function getJobAndPosterContact(jobId) {
  ...
  return {
    ...
    posterName: (profile && (profile.companyName || profile.agencyName)) || 'a Kezera employer',
...
function buildRosterMatchEmail({ jobTitle, posterName, candidateNames }) {
  ...
  const htmlBody = `
    ...
    <p>Log in to your Kezera dashboard to view the details.</p>
  `.trim();
...
function buildPosterEmail({ count, jobTitle, ownRosterNames = [] }) {
  ...
  const htmlBody = `
    ...
    <p>Job: <strong>${jobTitle}</strong></p>
    ${rosterLine}
    <p>Log in to your Kezera dashboard to view the shortlist.</p>
  `.trim();
  return { subject, htmlBody };
}
...
function buildInterviewSmsMessage(event, { jobTitle, posterName, when, mode }) {
  ...
  return `Interview scheduled: "${jobTitle}" with ${posterName} on ${when} (${modeLabel}). Check your Kezera app for details.`;
}
...
function buildInterviewEmail(event, {
  ...
  if (event === 'cancelled') {
    const htmlBody = `
      ...
      <p>Log in to your Kezera dashboard for more details.</p>
    `.trim();
    return { subject, htmlBody };
  }
...
async function notifyNewMessage(messageId) {
  ...
  const smsMessage = `New message about "${job.title}" on Kezera. Log in to view and reply.`;
    ...
    try {
      await sendEmail({
        ...
        htmlBody: `
          ...
          <p>Log in to your Kezera dashboard to view and reply.</p>
        `.trim(),
      });
      emailSent = true;
    } catch (err) {
...
async function notifyNewApplication(applicationId) {
  ...
  const smsMessage = `${applicantName} applied for "${job.title}" on Kezera. Log in to view their profile.`;
    ...
    try {
      await sendEmail({
        ...
        htmlBody: `
          ...
          <p>Log in to your Kezera dashboard to view their profile.</p>
        `.trim(),
      });
      emailSent = true;
    } catch (err) {
...
function buildVerificationRejectedSmsMessage(reason, refund) {
  const base = `Your Kezera verification was not approved: ${reason}`;
...
async function notifyVerificationRejected(userId, reason, refund = { status: 'none' }) {
  ...
  if (user.email) {
    try {
      ...
      await sendEmail({
        to: user.email,
        subject: 'Your Kezera verification was not approved',
        htmlBody: `
          <p>Hi ${accountName},</p>
...
async function notifyAccountStatusChanged(userId, status, reason) {
  ...
  const smsMessage = status === 'suspended'
    ? `Your Kezera account has been suspended by an administrator.${reason ? ` Reason: ${reason}.` : ''} Contact support if you believe this is a mistake.`
    : 'Your Kezera account has been reactivated. You can resume using the platform normally.';
    ...
    try {
      await sendEmail({
        ...
        subject: status === 'suspended'
          ? 'Your Kezera account has been suspended'
          : 'Your Kezera account has been reactivated',
        htmlBody: `
          <p>Hi ${accountName},</p>
          <p>${smsMessage}</p>
        `.trim(),
...
/**
 * "Request Service": one SMS/email/in-app message for every stage of a
 * ServiceRequest's lifecycle. Unlike notifyNewMessage above, the
 * recipient is passed in explicitly rather than re-derived from the
 * record here — serviceRequests.service.js already knows exactly who
 * the "other side" of each transition is (the requester for an
 * accept/decline/complete/cancel a Expert or agency triggered, or the
 * Expert/agency for a brand-new/just-assigned request), so there's no
 * ambiguity to resolve worth a second lookup. Mirrors
 * notifyAccountStatusChanged's "explicit userId" shape rather than
 * notifyNewMessage's "derive it from the record" shape.
 *
 ...
 * @returns {Promise<{ sent: boolean, smsSent: boolean, emailSent: boolean }>}
 */
async function notifyServiceRequestEvent(serviceRequestId, event, recipientUserId) {
  const serviceRequest = await ServiceRequest.findById(serviceRequestId).select('title');
  if (!serviceRequest) {
    // eslint-disable-next-line no-console
    console.error(`[notifications.service] service request ${serviceRequestId} not found for notification`);
    return { sent: false, smsSent: false, emailSent: false };
  }
  ...
  const { title } = serviceRequest;
  const messagesByEvent = {
    created: `New service request: "${title}". Log in to Kezera to respond.`,
    assigned: `You've been assigned a service request: "${title}". Log in to respond.`,
    accepted: `Your service request "${title}" was accepted.`,
    declined: `Your service request "${title}" was declined.`,
    completed: `Your service request "${title}" was marked completed.`,
    cancelled: `Your service request "${title}" was cancelled.`,
  };
  const smsMessage = messagesByEvent[event] || `Update on your service request "${title}".`;
  const isActionNeeded = event === 'created' || event === 'assigned';
  ...
  if (user.phone) {
    try {
      ...
      // eslint-disable-next-line no-console
      console.error(`[notifications.service] service_request_id=${serviceRequestId} event=${event} SMS failed:`, err.message);
    }
  }
  ...
  if (user.email) {
    try {
      await sendEmail({
        ...
        htmlBody: `<p>${smsMessage}</p>`,
      });
      emailSent = true;
    } catch (err) {
      // eslint-disable-next-line no-console
      console.error(`[notifications.service] service_request_id=${serviceRequestId} event=${event} email failed:`, err.message);
    }
  }

  await writeInAppNotification(
    recipientUserId,
    isActionNeeded ? 'service_request_new' : 'service_request_update',
    isActionNeeded ? `New service request: "${title}"` : `Service request update: "${title}"`,
    smsMessage,
    { serviceRequestId, event },
  );
