 ...
 *
 * @param {string} agencyUserId - req.user.id (JWT sub)
 * @param {{ agency_name?: string, operational_city?: string, backoffice_phone?: string, address?: string, bio?: string }} updates - already Joi-validated
 */
...
/**
 * PATCH /api/v1/agencies/me/location
 * "Find agencies near you": stores the agency's office coordinates and
 * keeps the `location` GeoJSON mirror in sync, mirroring
 * seeker.service.js#updateLocation exactly (see that function for the
 * full rationale, including the [lng, lat] GeoJSON order note).
 *
 * @param {string} agencyUserId
 * @param {{ latitude: number, longitude: number }} coords - already Joi-validated
 */
async function updateLocation(agencyUserId, coords) {
...
const NEARBY_AGENCIES_MAX_RESULTS = 100;

/**
 * GET /api/v1/agencies/nearby (no auth — "Find agencies near you" map,
 * mirroring `GET /seekers/nearby`). Given a point (the client's current
 * GPS fix, or a manually-dropped pin), returns agencies within
 * `radiusKm`, nearest first, each annotated with its count of
 * currently open job listings so the map/list can show "Acme Staffing
 * · 4 open roles" without a second round trip per pin. Also returns
 * `backofficePhone`/`address` (when the agency has set them) so the
 * quick-view sheet's Call/Directions actions have something to act on.
 *
 * Visibility mirrors `agencyComment.service.js#listAgencies` exactly —
 * same `User.verificationStatus === 'approved' && accountStatus ===
 * 'active'` gate (via `$lookup`, since that lives on the User doc, not
 * Agency), and the same `id: $toString('$userId')` convention, since
 * every other public agency route (`/agencies/:agencyId/profile`,
 * `/agencies/:agencyId/jobs`, ...) keys `agencyId` off the *User* id,
 * not the Agency document's own `_id`. Only agencies who have set an
 * office location are eligible — see the sparse 2dsphere index on
 * Agency.model.js; a document with no `location` simply can't match a
 * `$geoNear` stage. `$geoNear` (rather than `find().near()`, used on
 * the Seeker side) is used here specifically because it can lead an
 * aggregation pipeline, which is what makes joining straight to User
 * and Job in the same query possible, and it returns `distanceMeters`
 * for free instead of a separate haversine pass.
 *
 * @param {{ latitude: number, longitude: number, radiusKm: number, limit: number }} filters
 */
async function nearbyAgencies(filters) {
  const { latitude, longitude, radiusKm, limit } = filters;
  const requestedLimit = Math.min(limit, NEARBY_AGENCIES_MAX_RESULTS);
  ...
  const agencies = await Agency.aggregate([
    ...
    { $limit: requestedLimit },
    {
      $lookup: {
        ...
        let: { agencyUserId: '$userId' },
        pipeline: [
          {
            $match: {
              $expr: {
                $and: [
                  { $eq: ['$creatorId', '$$agencyUserId'] },
                  { $eq: ['$creatorType', 'agency'] },
                  { $eq: ['$status', 'open'] },
                ],
              },
            },
    ...
    },
    {
      $project: {
        ...
        openJobsCount: { $ifNull: [{ $arrayElemAt: ['$openJobs.count', 0] }, 0] },
      },
    },
  ]);

  return { agencies, count: agencies.length, radiusKm };
}
