...
const {
  Job, Employer, Agency, Application,
} = require('../models');
 ...
 * Reporting summary for the employer/agency dashboard: how many jobs
 * this account has posted (by status), how many applicants have come
 * in across all of them combined, how many of those arrived today, and
 * a short "recent applicants" feed so the dashboard has something
 * concrete to show alongside the counts. Deliberately a separate
 * endpoint from [getMyJobs] rather than fields tacked onto each job —
 * the dashboard wants account-wide totals, not a per-job breakdown,
 * and computing this from the full job list client-side would mean
 * fetching every job's applications individually.
 *
 * @param {string} creatorId - req.user.id
 */
async function getMyJobsStats(creatorId) {
  const jobs = await Job.find({ creatorId }).select('title status').lean();
  ...
  // Local midnight, not UTC — "applied today" should match what the
  // employer/agency actually experiences as "today", the same
  // convention the rest of the reporting surface (e.g.
  // admin.service.js#getPlatformStats' "this month") uses.
  const startOfToday = new Date();
  startOfToday.setHours(0, 0, 0, 0);

  const [totalApplicants, newApplicantsToday, recentApplications] = await Promise.all([
    Application.countDocuments({ jobId: { $in: jobIds } }),
    Application.countDocuments({ jobId: { $in: jobIds }, createdAt: { $gte: startOfToday } }),
    // A handful of the most recent applicants across every job this
    // account owns, for the dashboard's "recent applicants" list —
    // not a full listing (that's GET /jobs/:id/applications, per job).
  ...
  const recentApplicants = recentApplications.map((doc) => {
    ...
    return {
      ...
      applicantName: seeker ? seeker.fullName : 'Unknown candidate',
      // Needed so the dashboard's "Recent applicants" tile can start a
      // chat directly (POST /jobs/:id/invite-candidate takes jobId +
      // seekerId) without a second round trip to look it up. `seeker`
      // is null if the seeker account was deleted after applying —
      // that row just can't be messaged from here.
      seekerId: seeker ? seeker.id : null,
      appliedAt: doc.createdAt,
      isNewToday: doc.createdAt >= startOfToday,
    };
  });
 ...
 * JS-06: "For You" personalized feed)
 ...
 *   categories?: string[], creator_type?: 'employer'|'agency',
 *   experience_level?: string, page: number, limit: number
...
async function browseJobs(filters) {
  const {
    keyword, location, job_type: jobType, category, categories, creator_type: creatorType,
  ...
  // "For You" feed (JS-06): `categories` (the seeker's full preferred
  // set, from `preferredCategories` — see seeker.controller.js's
  // caller) takes priority over the single `category` filter when both
  // are present, matching a job posted under ANY of them rather than
  // requiring an exact single match.
