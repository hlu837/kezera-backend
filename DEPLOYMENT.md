...
## 7. No migration required to deploy — one optional cleanup, whenever convenient

Nothing here blocks deploying the Phase 6 (Expert/Technician split)
code changes. Mongoose no longer declaring `tradeCategory`/lat/lng/
`location` on `Seeker.model.js` doesn't require touching existing
data first — those fields simply become invisible to every
route/service in this codebase the moment the new code is live, since
Mongoose only hydrates paths still declared in the schema.

That said, any Seeker who had those fields set before Phase 6 still
has that data physically sitting in MongoDB (already copied into a
`Technician` document back in Phase 3's migration), plus a
`location_2dsphere` index that's now unused — `autoIndex` creates
indexes still declared in the schema, but never drops ones that
aren't. Purely hygiene, not urgent:

```bash
npm run cleanup:seeker-expert-fields            # apply
npm run cleanup:seeker-expert-fields -- --dry-run  # preview first
```

Safe to run more than once, and safe to run before or after the
Vercel deploy — it only touches data, not schema/code, so ordering
doesn't matter. Needs the same `MONGODB_URI` as everything else, run
from wherever you can reach Atlas (locally, or one-off from whatever
platform hosts the workers).
